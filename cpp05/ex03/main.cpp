...
реализовать метод makeForm() первым парметром принимает 
имя формы, вторым цель этой формы.
метод возвращает указатель на конкретную форму.

выведет на стандартный вывод что-то вроде"Стажер создает <форму>".
если он использует худший метод, например, ветвления if/elseif/elseif/else, 
или какие-то другие уродливые вещи, подобные этому, он не будет принят во время оценки.

если запрошенная форма неизвестна, вывести явное сообщение об ошибке.
*/
